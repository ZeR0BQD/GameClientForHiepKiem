...
namespace GameClient
{
    ...
    public class AutoPathManager
    {
        ...
        /// Lớp quản lý tự tìm đường
        /// </summary>
        public static AutoPathManager Instance { get; set; }
        #endregion
        ...
        {
            //XElement xmlNode = Global.ReadXMLData("Config/AutoPath.xml");
            AutoPathManager.Instance = new AutoPathManager();
            //AutoPathManager.Instance.AutoPaths = AutoPathXML.Parse(xmlNode);
        ...
        /// <returns></returns>
        public bool CanTransferFromNPCToMap(RoleData player, int toMapCode, out AutoPathXML.Node node)
        {
            int currentMapCode = player.MapCode;
            int posX = (int)player.PosX;
            int posY = (int)player.PosY;
            Vector2 fromPos = new Vector2(posX, posY);
            node = this.AutoPaths.TransferNPCs.Where((nodeInfo) => {
                if (nodeInfo.FromMapCode != player.MapCode || nodeInfo.ToMapCode != toMapCode)
                {
                    return false;
                }
                Vector2 npcPos = new Vector2(nodeInfo.PosX, nodeInfo.PosY);
                return Vector2.Distance(fromPos, npcPos) <= 100;
            }).FirstOrDefault();
        ...
        /// <returns></returns>
        public bool CanTransferByUsingTeleportItem(RoleData player, int itemID, int toMapCode, out AutoPathXML.Node node)
        {
            /// Mặc định Node NULL
            node = null;
            ...
            node = paths.Where((nodeInfo) => {
                if ((nodeInfo.FromMapCode != 0 && nodeInfo.FromMapCode != player.MapCode) || nodeInfo.ToMapCode != toMapCode)
                {
                    return false;
                }
                Vector2 pos = new Vector2(nodeInfo.PosX, nodeInfo.PosY);
                ...
                {
                    return Vector2.Distance(fromPos, pos) <= 100;
                }
            }).FirstOrDefault();
